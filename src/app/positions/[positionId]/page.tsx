...
import { Application, PipelineStageKey, Position as PositionType } from '@/lib/types';
...
const CandidateCard: React.FC<CandidateCardProps> = ({ application }) => {
  ...
  const { candidateSnapshot, candidateId } = application;
    ...
    >
      <CardContent className="p-4 flex items-center gap-3">
        <div className="flex-shrink-0 h-10 w-10 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 text-sm font-medium">
          {candidateSnapshot.fullName ? candidateSnapshot.fullName.charAt(0).toUpperCase() : 'N/A'}
        </div>
        <div className="flex-grow">
          <p className="text-sm font-medium text-slate-900">{candidateSnapshot.fullName}</p>
          {candidateSnapshot.currentTitle && <p className="text-xs text-slate-500">{candidateSnapshot.currentTitle}</p>}
          {application.appliedDate && (
            <p className="mt-1 text-xs text-slate-400">
              {application.appliedDate.toDate().toLocaleDateString()}
            </p>
          )}
        </div>
      </CardContent>
...
const PositionDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPositionData = async () => {
      ...
      if (!foundPosition) {
        // If not in global store, try fetching directly
        foundPosition = await positionsRepository.getPositionById(positionId);
        if (foundPosition) {
          // If found, add/update in global store
  ...
  useEffect(() => {
    if (currentPosition && clientsInitialized) {
      // Ensure client-specific positions are loaded if not already
      // This is primarily for the client detail page, but ensures all related data is there
  ...
  const statusConfig =
    POSITION_STATUS_CONFIG[currentPosition.status.toLowerCase()] ||
  ...
  (applicationsByPosition[positionId] || []).forEach((app) => {
    let stageKey: PipelineStageKey | undefined = app.stageKey;

    if (!stageKey && app.status) { // Backward compatibility
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-4 text-sm text-slate-500">
            ...
            {(currentPosition.location || client?.location) && (currentPosition.salaryRange || (currentPosition.minSalary && currentPosition.maxSalary)) && <span className="text-slate-400">&bull;</span>}
            {(currentPosition.salaryRange || (currentPosition.minSalary && currentPosition.maxSalary)) ? (
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Button variant="secondary" className="h-8 text-sm px-3" disabled> {/* Disabled for now */}
